...
async function getStats() {
    try {
        ...
        const tierA = await prisma.opportunity.count({ where: { tier: 'A' } });
        const won = await prisma.place.count({ where: { status: 'WON' } });
        const newOpportunities = await prisma.place.count({ where: { status: 'NEW' } });

        return { total, tierA, won, newOpportunities };
    } catch (e) {
        return { total: 0, tierA: 0, won: 0, newOpportunities: 0 };
    }
}
            ...
            <div className="relative z-10 max-w-4xl w-full text-center space-y-8">
                <div className="space-y-2">
                    ...
                    <h1 className="text-7xl font-black tracking-tight bg-gradient-to-b from-white to-slate-500 bg-clip-text text-transparent">
                        Opportunity<span className="text-indigo-500">Gen</span>
                    </h1>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full">
                    ...
                    <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-2xl flex flex-col items-center hover:bg-slate-800 transition-all">
                        ...
                        <span className="text-xs uppercase tracking-wider text-emerald-500/50 font-bold">High Value</span>
                    </div>
                    <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-2xl flex flex-col items-center hover:bg-slate-800 transition-all">
                        <span className="text-4xl font-bold text-blue-400 mb-2">{stats.newOpportunities}</span>
                        <span className="text-xs uppercase tracking-wider text-blue-500/50 font-bold">New Targets</span>
                    </div>
                    <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-2xl flex flex-col items-center hover:bg-slate-800 transition-all">
                ...
                <div className="pt-8">
                    <Link href="/opportunities" className="inline-flex items-center gap-3 bg-white text-slate-950 px-8 py-4 rounded-xl font-bold text-lg hover:scale-105 active:scale-95 transition-all shadow-xl shadow-white/10 group">
                        Launch Workbench
